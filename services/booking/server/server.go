...
func StartGrpcServer(s *service.BookingService, logger *utils.Logger) error {
	...
	booking.RegisterBookingServiceServer(server, s)

	go gracefulShutdown(server, lis, logger)
	logger.Info("Booking Service Ready")
	logger.Info("Booking Service gRPC server listening on %s", addr)
